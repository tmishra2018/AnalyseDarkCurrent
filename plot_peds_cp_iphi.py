...
if not subtractQIE:
    ...
    outFile=ROOT.TFile("hists/hists_sub_"+filename+"_negieta_iphi.root","RECREATE")
...
h1_3p3_oth = ROOT.TH1F("h1_3p3_oth_"+run,";Mean 8TS pedestal sum ["+unit+"]; Number of channels",nbins,minx,maxx)
h1_3p30_oth = ROOT.TH1F("h1_3p30_oth_"+run,";Mean 8TS pedestal sum ["+unit+"]; Number of channels",nbins,minx,maxx)
h1_3p31_oth = ROOT.TH1F("h1_3p31_oth_"+run,";Mean 8TS pedestal sum ["+unit+"]; Number of channels",nbins,minx,maxx)
h1_3p32_oth = ROOT.TH1F("h1_3p32_oth_"+run,";Mean 8TS pedestal sum ["+unit+"]; Number of channels",nbins,minx,maxx)
...
h1_3p3_oth.StatOverflows(True)
h1_3p30_oth.StatOverflows(True)
h1_3p31_oth.StatOverflows(True)
h1_3p32_oth.StatOverflows(True)
...
for x in range(1,1+h2.GetNbinsX()):
    for y in range(1,1+h2.GetNbinsY()):
        if(h2.GetBinContent(x,y)>0):
            ...
            is3p3 = (iphi>34 and iphi<=38)
            is3p30 = (iphi>34 and iphi<=38 and depth==2)#sp31
            is3p31 = (iphi>34 and iphi<=38 and depth==3)#sp31
            is3p32 = (iphi>34 and iphi<=38 and depth==5)
            is3p3_oth = (iphi<=34 or iphi>38)
            is3p30_oth = ((iphi<=34 or iphi>38) and depth==2)#sp31
            is3p31_oth = ((iphi<=34 or iphi>38) and depth==3)#sp31
            is3p32_oth = ((iphi<=34 or iphi>38) and depth==5)
            if is3p3:
                h1_3p3.Fill(mean)
            elif is3p3_oth:
                h1_3p3_oth.Fill(mean)
            if is3p30:                
                h1_3p30.Fill(mean)
                #print 'Eta '+str(eta)+ ' depth ' +str(depth)+' iphi '+ str(iphi)+ ' charge '+str(mean) 
            elif is3p30_oth:
                h1_3p30_oth.Fill(mean)
                #print 'Eta '+str(eta)+ ' depth ' +str(depth)+' iphi '+ str(iphi)+ ' charge '+str(mean) 
            elif is3p31:                
                h1_3p31.Fill(mean)
                #print 'Eta '+str(eta)+ ' depth ' +str(depth)+' iphi '+ str(iphi)+ ' charge '+str(mean) 
            elif is3p31_oth:
                h1_3p31_oth.Fill(mean)
            elif is3p32:
                h1_3p32.Fill(mean)
                #print 'Eta '+str(eta)+ ' depth ' +str(depth)+' iphi '+ str(iphi)+ ' charge '+str(mean) 
            elif is3p32_oth:
                h1_3p32_oth.Fill(mean)
