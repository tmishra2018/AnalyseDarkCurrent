...
for iphi in range(miniphi, miniphi+niphi):
    ...
    #outFile=ROOT.TFile("hists/iphi/hists_sub_"+filename+"_posieta_iphi_"+str(iphi)+".root","RECREATE")
    outFile=ROOT.TFile("hists/iphi/hists_sub_"+filename+"_negieta_iphi_"+str(iphi)+".root","RECREATE")

    h1 = ROOT.TH1F("h1_"+str(iphi)+"_"+run,";Mean 8TS pedestal sum ["+unit+"]; Number of channels",nbins,minx,maxx)
    h1_3p3 = ROOT.TH1F("h1_3p3_"+str(iphi)+"_"+run,";Mean 8TS pedestal sum ["+unit+"]; Number of channels",nbins,minx,maxx)
    h1_2p8 = ROOT.TH1F("h1_2p8_"+str(iphi)+"_"+run,";Mean 8TS pedestal sum ["+unit+"]; Number of channels",nbins,minx,maxx)

    h1e = ROOT.TH1F("h1e_"+str(iphi)+"_"+run,";Error on mean 8TS pedestal sum ["+unit+"]; Number of channels",12,0,6)
    h1e_2p8 = ROOT.TH1F("h1e_2p8_"+str(iphi)+"_"+run,";Error on mean 8TS pedestal sum ["+unit+"]; Number of channels",12,0,6)
    h1e_3p3 = ROOT.TH1F("h1e_3p3_"+str(iphi)+"_"+run,";Error on mean 8TS pedestal sum ["+unit+"]; Number of channels",12,0,6)
    ...
    for x in range(1,1+h2.GetNbinsX()):
        for y in range(1,1+h2.GetNbinsY()):
            if(h2.GetBinContent(x,y)>0):
                ...
                iphi_l = (yvalue-miniphi)%niphi+miniphi
                depth = (yvalue-iphi_l)/72
                #if(iphi_l>=55 and iphi_l<=65):
                 #   print 'SiPM run '+run+' Eta '+str(eta)+ ' iphi '+ str(iphi)+' iphi_l '+str(iphi_l)+' depth ' +str(depth)+ ' charge '+str(mean)
