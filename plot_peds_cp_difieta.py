...
if not subtractQIE:
    ...
    outFile=ROOT.TFile("hists/hists_sub_"+filename+"_negieta18_50fb.root","RECREATE")
...
for x in range(1,1+h2.GetNbinsX()):
    for y in range(1,1+h2.GetNbinsY()):
        if(h2.GetBinContent(x,y)>0):
            ...
            is3p30 = (eta==18 and depth==1 and iphi==62)#sp31
            is3p31 = (eta==17 and depth==2 and iphi==35)#sp31
            is3p32 = (eta==18 and depth==5 and iphi==62)
            if is3p30:
                #print 'Eta '+str(eta)+ ' depth ' +str(depth)
                h1_3p30.Fill(mean)
            elif is3p31:                
                print 'eta '+str(eta)+ ' depth ' +str(depth)+' iphi '+str(iphi)+' mean '+str(mean)
                h1_3p31.Fill(mean)
            elif is3p32:
                print 'eta '+str(eta)+ ' depth ' +str(depth)+' iphi '+str(iphi)+' mean '+str(mean)
                h1_3p32.Fill(mean)
