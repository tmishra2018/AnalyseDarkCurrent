...
if not subtractQIE:
    ...
    outFile=ROOT.TFile("hists/hists_sub_"+filename+"_negieta_sub_"+subname_r+".root","RECREATE")
    #outFile=ROOT.TFile("hists/hists_sub_"+filename+"_negieta_sub_330959.root","RECREATE")
    #outFile=ROOT.TFile("hists/hists_sub_"+filename+"_negieta_sub_315068.root","RECREATE")
    #outFile=ROOT.TFile("hists/hists_sub_"+filename+"_ieta23.root","RECREATE")
print 'filename: ' +filename
print 'run '+run
#outFile=ROOT.TFile("hists_"+filename+".root","RECREATE")
print(ch.GetEntries())
print 'subtracting number '+subname_r
...
for x in range(1,1+h2.GetNbinsX()):
    for y in range(1,1+h2.GetNbinsY()):
        if(h2.GetBinContent(x,y)>0):
            ...
            print 'original run SiPM mean '+str(mean)+ ' qie mean '+str(q_h2.GetBinContent(x,y))
            if subtractQIE:
            ...
            #if(mean<100):
            print ' Eta' +str(eta) +' phi '+ str(iphi)+ ' depth ' + str(depth)+ ' mean subtraction ' + str(mean)
            isiphi=(iphi>30 and iphi<40)
            is3p3 = (eta>=-17 or (eta==-18 and depth==5))
            if is3p3:
                print '3.3mm SiPM Eta '+str(eta)+ ' iphi '+ str(iphi)+ ' depth ' +str(depth)+ ' charge '+str(mean)
                h1_3p3.Fill(mean)
            else:
                print '2.8mm SiPM Eta '+str(eta)+ ' iphi '+ str(iphi)+ ' depth ' +str(depth)+ ' charge '+str(mean)
                h1_2p8.Fill(mean)
